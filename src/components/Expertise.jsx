...
const TagCard = ({ number, title, text, className, aosDelay, aosType, pathLength, containerRef }) => {
  ...
  return (
    <div
      ...
      className={`w-72 sm:w-80 rounded-[2rem] p-2 relative flex flex-col items-center hover:scale-[1.02] transition-all duration-700 z-10 ${className} ${isActive
  ? 'bg-[#0B1120] border border-violet-500/40 shadow-[0_20px_50px_rgba(168,85,247,0.25)]'
  : 'bg-white border border-gray-200 shadow-[0_15px_40px_rgba(0,0,0,0.06)]'
    ...
    >
      ...
      {/* Inner container */}
     <div
  className={`w-full h-full rounded-[1.5rem] mt-8 p-8 flex flex-col min-h-[220px] transition-all duration-700 ${
    isActive
      ? 'bg-violet-400/35 border border-violet-300'
      : 'bg-[#f4f4f4]'
  }`}
>
        <span className={`text-xl font-bold mb-2 font-serif italic transition-colors duration-700 ${isActive ? 'text-purple-200' : 'text-gray-400'
          }`}>{number}</span>

        <h3 className={`text-2xl font-black mb-3 tracking-tight transition-colors duration-700 ${isActive ? 'text-white' : 'text-gray-900'
          }`}>{title}</h3>

        <p className={`text-sm leading-relaxed font-medium transition-colors duration-700 ${isActive ? 'text-purple-100' : 'text-gray-500'
          }`}>
          {text}
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto relative md:h-[1350px]">
        ...
        <div data-aos="fade-up" className="md:absolute top-10 left-0 md:w-[450px] z-20 mb-16 md:mb-0">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 leading-[1.1] mb-6 tracking-tight relative">
            Building Practical Software with Code, Logic & Curiosity
            {/* Hand-drawn arrow */}
          ...
          <p className="text-gray-500 text-base md:text-lg max-w-sm font-medium leading-relaxed">
            Focused on full-stack web development, DSA, OOP, REST APIs, databases, and AI-enabled projects that solve real problems.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-8 md:gap-12 items-center md:block relative z-10 w-full pt-4 md:pt-0 pb-12 md:pb-0">
          ...
          <TagCard
            ...
            text="Creating responsive interfaces with HTML5, CSS3, JavaScript, and React basics, with attention to clean layouts and smooth user experience."
          ...
          <TagCard
            ...
            text="Building REST APIs, authentication flows, and database-backed applications using Node.js, Express.js, FastAPI, MongoDB, and MySQL."
          ...
          <TagCard
            number="03"
            title="AI Projects"
            text="Exploring AI-based systems such as MockMate.AI, with response analysis, feedback generation, and personalized interview practice."
          ...
          <TagCard
            ...
            text="Practicing Data Structures, Algorithms, OOP, Operating Systems, DBMS, and Computer Networks for interviews and software engineering roles."
